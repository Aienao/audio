...
public class Config {
    ...
    private static Integer USER_UUID_MAX_AGE; // 用户uuid有效期
    ...
    public static Integer USER_UUID_MAX_AGE() {
        return USER_UUID_MAX_AGE;
    }
    ...
    public void init() {
        try {
            ...
            try {
                USER_UUID_MAX_AGE = Integer.parseInt(prop.getProperty("user-uuid.max-age"));
            } catch (Exception ex) {
                logger.error("cookie.max-age illegal!");
            }
            if (USER_UUID_MAX_AGE == null) {
                USER_UUID_MAX_AGE = 60 * 60 * 24 * 30;
            }
